const helper = {
  ...
  /**
   * Determines if specified component name is the "cat-*" component name.
   * @param {string} componentName - Name of the component.
   * @returns {boolean} True if specified component is the "cat-*" component.
   */
  isCatComponent (componentName) {
    return helper.COMPONENT_PREFIX_REGEXP.test(componentName);
  },
   ...
   */
  getMethodToInvoke (module, name, prefix) {
  ...
  /**
   * Check component descriptor
   * @param {*} descriptor
   * @returns {Boolean}
   */
  isValidDescriptor (descriptor) {
    return descriptor && descriptor.constructor && typeof descriptor === 'object';
  }
};
